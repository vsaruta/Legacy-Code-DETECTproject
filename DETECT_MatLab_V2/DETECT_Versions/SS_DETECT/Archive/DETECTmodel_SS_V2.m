...
%VVS SAVE INPUT DATA TO CSV FILES(so it may be used in other place)
% csvwrite('DD_Nt_run.csv', Nt_run); 
% csvwrite('DD_SWC.csv', SWC); 
% csvwrite('DD_SoilT.csv', SoilT);
% csvwrite('DD_SWCant_Rm.csv', SWCant_Rm); 
% csvwrite('DD_SWCant_Rr.csv', SWCant_Rr); 
% csvwrite('DD_SoilTant.csv', SoilTant); 
% csvwrite('DD_Gness.csv', Gness);
% csvwrite('DD_AtmPress.csv', AtmPress);
% csvwrite('DD_params_swp.csv', params_swp); 
% csvwrite('DD_Xrel.csv', Xrel); 
% csvwrite('DD_ic.csv', ic); 
% csvwrite('DD_d13Cr.csv', d13Cr); 
% csvwrite('DD_d13Cm.csv', d13Cm); 
% csvwrite('DD_dx.csv', dx); 
% csvwrite('DD_BC.csv', BC);
% csvwrite('DD_params.csv', params); 
% csvwrite('DD_params_lag.csv', params_lag); 
...
PD=2.52; 
SWC100=params_swp(3,:);
% SETTING SWC TO BE CONSTANT
% DIDN'T INFLUENCE MUCH
% SWC(:,:) =  mean(SWC(:));
phiT=1-(BD./PD);  %air-filled porosity
test1 = repmat(phiT,Nt,1);
test2 = repmat(phiT,Nt,1) - SWC;
...
% MAKING SoilT CONSTANT
% DIDN'T INFLUANCE
% SoilT(:,:) = mean(SoilT(:));
% VVS ADD LINE to check P0
csvwrite('P0_test.csv',P0);
T = SoilT + 273.15;
% VVS ADD LINE to check

% MAKING AtmPress CONSTANT
AtmPress(:,:) = mean(AtmPress(:));
P = repmat(AtmPress,1,Nz);  
% VVS ADD LINE to check P
csvwrite('P_test.csv',P);
logDg0= log(Dg0st)+(1.75*log(T/T0))+log(P0./P);
% VVS ADD LINE to check b
csvwrite('b_test.csv',b);
...
% These have no influance
% SWC(:,:) = mean(SWC(:));
% SoilT(:,:) = mean(SoilT(:));
% SWCant_Rm(:,:) = mean(SWCant_Rm(:));
% SWCant_Rr(:,:) = mean(SWCant_Rr(:));
% SoilTant(:,:) = mean(SoilTant(:));

Rall = SourceTerm(params,params_lag,Xrel,SWC,SoilT,SWCant_Rm,SWCant_Rr,SoilTant,Gness);
% VVS ADD LINE to check Rall
% csvwrite('Rall_test.csv', Rall);
%VVS SAVE INPUT DATA TO CSV FILES(so it may be used in other place)
% csvwrite('ST_params.csv', params); 
% csvwrite('ST_params_lag.csv', params_lag); 
% csvwrite('ST_Xrel.csv', Xrel);
% csvwrite('ST_SWC.csv', SWC); 
% csvwrite('ST_SoilT.csv', SoilT);
% csvwrite('ST_SWCant_Rm.csv', SWCant_Rm); 
% csvwrite('ST_SWCant_Rr.csv', SWCant_Rr); 
% csvwrite('ST_SoilTant.csv', SoilTant);
% csvwrite('ST_Gness.csv', Gness);



R = [zeros(Nt,1) Rall.R];
% VVS ADD LINE to check R
csvwrite('R_R.csv', R);
Rr= [zeros(Nt,1) Rall.Rr];
% VVS ADD LINE to check Rr
csvwrite('R_Rr.csv', Rr);
Rm= [zeros(Nt,1) Rall.Rm];
% VVS ADD LINE to check Rm
csvwrite('R_Rm.csv', Rm);
...
csvwrite('denomR_test.csv', denomR);
denomM = (1000+(1000+repmat(d13Cm,[Nt 1]))*Rstd);
csvwrite('denomM_test.csv', denomM);
S13root = Sr.*(1000+repmat(d13Cr,[Nt 1])).*Rstd./denomR;
csvwrite('S13root_test.csv', S13root);
S12root = 1000*Sr./denomR;
csvwrite('S12root_test.csv', S12root);
S13micr = Sm.*(1000+repmat(d13Cm,[Nt 1])).*Rstd./denomM;
csvwrite('S13micr_test.csv', S13micr);
S12micr = 1000*Sm./denomM;

Stype(1,:,:)=S13root;
% VVS ADD LINE to check Stype
csvwrite('Stype_1.csv', Stype);
Stype(2,:,:)=S12root;
% VVS ADD LINE to check Stype
csvwrite('Stype_2.csv', Stype);
Stype(3,:,:)=S13micr;
% VVS ADD LINE to check Stype
csvwrite('Stype_3.csv', Stype);
Stype(4,:,:)=S12micr;
% VVS ADD LINE to check Stype
csvwrite('Stype_4.csv', Stype);
Stype(:,:,1)=0;
% VVS ADD LINE to check Stype
csvwrite('Stype_5.csv', Stype);
...
csvwrite('cctype.csv', cctype);
fluxtype=zeros(4,Nt);
csvwrite('fluxtype.csv', fluxtype);
for i=1:4,
...
% VVS Alternative read in DATA
% csvwrite('Dgs_t_data.csv', Dgs_t);
% csvwrite('Stype_data.csv', Stype);
% csvwrite('catm_data.csv', catm);

% save('Stype_mat.mat', 'Stype');
...
info.Dgs_t=Dgs_t;
info.cctype_midle_check = cctype_midle_check;
