...
for (t in 1:Nt_run) {
  ...
  for (tt in 1:Ndt){
    ...
    for (z in 2:Nz) {
      ...
      temp <- numeric(101) 
      ctype <- foreach( i=1:4, # iteration index
                        .combine=rbind#, # specify how to combine iterations (index order is preserved)
                        # .export=c("ctype_old","New_DGS_1_3","New_DGS_2_4","Stype","dt","dz","t") 
      ...
      # for (i in 1:4) {
        ...
        if (z==Nz) {
          temp[z] = ctype_old[i,z] + dt*((Dgs_t[z]/(dz*dz))*(ctype_old[i,z-1]-ctype_old[i,z])+ Stype[i,t,z]);
        }
        else {
          temp[z] = ctype_old[i,z] + dt*((Dgs_t[z]/(dz*dz))*(ctype_old[i,z+1]-2*ctype_old[i,z]+ctype_old[i,z-1]) + ((Dgs_t[z+1]-Dgs_t[z-1])*(ctype_old[i,z+1]-ctype_old[i,z-1]))/(4*dz^2) + Stype[i,t,z]);
        }
          temp
      }
    }
